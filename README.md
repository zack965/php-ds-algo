...
`Zack\PhpDsAlgo\Algorithmes\ArraySearchAlogorthme` — all six methods return the found index/key, or `-1` if not found. `binarySearch()`, `exponentialSearchImplementation()`, `interpolationSearchRecursive()`, `jumpSearch()`, and `TernarySearchAlgorythme()` all require a **sorted** array; `linearSearch()` does not.
